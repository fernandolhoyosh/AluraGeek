...
async function listarProductos() {
    try {
        const conexion = await fetch("https://api-alurageek-server-ozv3maruh.vercel.app");
...
async function enviarProductoEliminar(id) {
    try {
        const conexion = await fetch(`https://json-server-api-alurageek-5lbi94ysj.vercel.app/productos/${id}`, {
            method: "DELETE"
        });
        const data = await conexion.json();
        return data;
...
async function buscarProducto(palabra) {
    try {
        const conexion = await fetch(`https://json-server-api-alurageek-5lbi94ysj.vercel.app/productos?q=${palabra}`);
        const data = await conexion.json();
        return data;
    } catch (error) {
